...
final class DocumentWindowController: NSWindowController {
    ...
    func showAndStart() {
        ...
        loadStatusPage(title: "Opening \(documentURL.lastPathComponent)", message: "Starting the local PAD runner.")
        startRunner()
    }
    ...
    private func startRunner() {
        do {
            _ = try CanvasDocument.validate(documentURL)
            let bunURL = try BunResolver().resolve()
            let command = try RunnerCommandBuilder.command(
                bunURL: bunURL,
                documentURL: documentURL,
                searchStartURL: Bundle.main.bundleURL
            )

            try openLog()
            writeLog("Canvas opening \(documentURL.path)\n")
            ...
            process.environment = try CanvasRunnerEnvironment.environment(documentURL: documentURL)
            ...
            showError(title: "Could Not Open Canvas", message: String(describing: error))
    ...
    private func handleRunnerLine(_ line: String) {
        guard !didLoadRunnerURL, let openURL = CanvasOpenURL.extract(from: line) else {
            return
        }

        do {
            let safeURL = try CanvasOpenURL.validate(openURL)
    ...
    private func handleRunnerTermination(_ process: Process) {
        ...
        if !didLoadRunnerURL {
            showError(
                ...
                message: "The PAD runner exited with status \(process.terminationStatus) before printing a loopback HTTP(S) URL."
            )
        }
    }
    ...
    private func handleStartupTimeout() {
        ...
        showError(
            ...
            message: "The PAD runner did not print a loopback HTTP(S) URL within 20 seconds."
        )
        stopRunner()
    }
    ...
    private func openLog() throws {
        let logsDirectory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Logs/Canvas", isDirectory: true)
        ...
        let logURL = logsDirectory.appendingPathComponent("\(baseName)-\(timestamp).log", isDirectory: false)
